...
def training_loop(
    ...
    print("DATASET PARAMS", training_set_kwargs)
    ...
    # if only training for masks 
    # TODO add option to continue training from masks
    ...
    if TRAIN_IMG_OR_GLS_OR_MASK == "mask" and False: 
        ...
        #for block in G.synthesis.layers:
        
        # TODO check if frozen
        print("Unfreeze SEG layers")
        for name, layer in G.synthesis.named_modules():
            #print(name)
            #print("...")
            #print(layer)

            # check if this works somehow 
            if "toseg" in name: 
                #print("====")
                print(name)
                #print(layer)
                layer.required_grad = True
            #block = getattr(layer, block_names[tmp_i])
            #toseg = getattr(block, 'toseg')
            #toseg.required_grad = True
            #print(getattr(layer, "b4")) # gets the correct layer so then just access toseg and enable grad there
            #break
    #exit()
    ...
    while True:
        ...
        if (snapshot_data is not None) and (len(metrics) > 0):# and cur_tick != 0:
            ...
            for metric in metrics:
                print("-- Compute metric:", metric, " of ", len(metrics))
